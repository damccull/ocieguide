...
pub fn dist_binary() -> Result<(), anyhow::Error> {
    ...
    #[allow(unused_mut)]
    let mut distributable = project_root().join("target/release/ocieguide");

    #[allow(unused_mut)]
    let mut destination = dist_dir().join("ocieguide");
    ...
    // Copy config files
    let config_src = project_root().join("website/configuration");
...
pub fn dist_manpage() -> Result<(), anyhow::Error> {
    let page = Manual::new("ocieguide")
        .about("A website for US Army Soldiers to reference their equipment.")
        .render();
    fs::write(dist_dir().join("ocieguide.man"), page)?;
    Ok(())
}
