...
bunyan level=LOG_LEVEL:
    RUST_LOG="INFO,ocieguide={{level}}" cargo run --features=bunyan | bunyan
...
# run the program
run level=LOG_LEVEL:
    RUST_LOG="INFO,ocieguide={{level}}" cargo run
...
watch level=LOG_LEVEL:
    RUST_LOG="INFO,ocieguide={{level}}" cargo watch -x check -x "run"

# cargo watch with bunyan tracing
watch-bunyan level=LOG_LEVEL:
    RUST_LOG="INFO,ocieguide={{level}}" cargo watch -x check -x "run --features=bunyan | bunyan"
...
tokioconsole level=LOG_LEVEL:
    RUSTFLAGS="--cfg tokio_unstable" RUST_LOG="INFO,ocieguide={{level}}" cargo run --features=tokio-console
